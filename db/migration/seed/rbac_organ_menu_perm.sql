...
--
-- 背景：
--   1. admin_basic_organ 统一作为「组织管理」入口；机构为主实体，部门树在
--      同一前端页面按所选机构维护。部门权限保留为页面内操作权限。
...
-- ============================================================
-- 一、组织管理：admin_basic_organ 是唯一页面入口
-- ============================================================
UPDATE `system_menu`
SET `menu_name` = '组织管理', `remark` = '机构及部门管理', `icon` = 'OfficeBuilding', `sort_order` = 4
WHERE `menu_code` = 'admin_basic_organ';

-- 部门操作已并入组织管理页面，历史独立菜单不再展示；保留其权限记录供页面内按钮鉴权。
UPDATE `system_menu`
SET `is_visible` = 0, `status` = 0, `remark` = '已并入组织管理'
