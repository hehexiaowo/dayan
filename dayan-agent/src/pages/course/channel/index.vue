...
/**
 * 渠道课程列表页（course_source=2，与数据库 course 域同频）。
 *
 * - 布局仿微信公众号历史消息：头条大卡（封面 + 标题叠加 + 白底 meta 行），
 *   其余条目左文右缩略图；无图片字段，封面用板块色渐变 + 标题首字代替；
 * - 板块渲染差异：渠道=视频样式（播放钮/播放量）；
 * - 导航栏标题由 pages.json 承载（渠道课程）。
 */

/** 板块渲染语义：视频样式 + 播放量 */
const board = { key: 'channel', title: '渠道课程', icon: '渠', source: CourseSource.CHANNEL } as const;
...
const isVideo = computed(() => board.key === 'channel');
...
function onItemClick(item: Course) {
  ...
  uni.navigateTo({ url: `/pages/course/channel/detail?code=${item.courseCode}` });
}
