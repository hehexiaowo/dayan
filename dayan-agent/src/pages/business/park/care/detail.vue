        ...
        <view v-if="currentTabKey === 'base'" class="tab-panel">
          <view class="stat-grid">
            <view v-if="park.totalBeds != null" class="stat-item">
              <text class="stat-value orange">{{ park.totalBeds }}</text>
            ...
            <view v-if="park.nurseCount != null" class="stat-item">
              <text class="stat-value orange">{{ park.nurseCount }}</text>
...
const visibleBlocks = computed(() =>
  (detail.value?.displayBlocks || []).filter(
    (b) => !b.networkTags?.length || b.networkTags.includes('care'),
  ),
);
...
/** Banner 图片：优先从 careConfig.banners 取，fallback 到全部 type=1 图 */
const NETWORK = 'care' as const;
