<template>
  <view class="province-page">
    ...
    <view class="map-section">
      <!-- #ifdef H5 -->
      <div id="sojourn-province-map" class="map-container"></div>
      <!-- #endif -->
      <!-- #ifndef H5 -->
    ...
    <view class="table-section">
      ...
      <DyEmpty v-if="!loading && !regions.length" text="该省份暂无旅居机构" icon="空" color="gray" />
    </view>
  </view>
</template>
...
async function initChart() {
  // #ifdef H5
  const container = document.getElementById('sojourn-province-map');
  if (!container) return;
  ...
  try {
    ...
    console.warn('[sojourn/province] 省份 GeoJSON 加载失败:', provinceCode.value);
    return;
  }
  ...
  const mapName = 'sojourn-province-' + provinceCode.value;
...
function handleNavigate(cityOrDistrictCode: string) {
  if (isMuni.value) {
    uni.navigateTo({
      url: `/pages/business/park/sojourn/district?category=sojourn&provinceCode=${provinceCode.value}&cityCode=${provinceCode.value.substring(0, 2) + '0100'}&districtCode=${cityOrDistrictCode}`,
    });
  } else {
    uni.navigateTo({
      url: `/pages/business/park/sojourn/city?category=sojourn&provinceCode=${provinceCode.value}&cityCode=${cityOrDistrictCode}`,
    });
  }
}
