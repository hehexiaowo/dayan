<template>
  <view class="city-page">
    ...
    <view class="location-bar">
      <text class="location-text">旅居养老 · 选择区县</text>
    </view>
    ...
    <template v-else>
      ...
      <view class="stats-bar">
        <text class="stats-text">{{ districts.length }} 个区县 · {{ totalParks }} 家旅居机构</text>
      </view>
      ...
      <view class="district-list">
        ...
        >
          <view class="district-info">
            ...
            <text class="district-count">{{ item.count }} 家旅居机构</text>
          </view>
          <view class="district-right">
            <text class="arrow">›</text>
          </view>
        </view>
        <DyEmpty v-if="!districts.length" text="该城市暂无旅居机构" icon="空" color="gray" />
      </view>
    </template>
  </view>
</template>
...
function onDistrictClick(item: RegionItem) {
  uni.navigateTo({
    url: `/pages/business/park/sojourn/district?category=sojourn&provinceCode=${provinceCode.value}&cityCode=${cityCode.value}&districtCode=${item.code}`,
  });
}
