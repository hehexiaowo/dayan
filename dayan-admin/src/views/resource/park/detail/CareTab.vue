...
// ---------- 展开行 price ----------
const priceMap = ref<Map<string, ParkCarePrice[]>>(new Map())
...
async function loadPrices(row: ParkCareType) {
  ...
  try {
    const list = await listCarePrices(props.parkCode, row.careTypeCode)
...
const priceForm = reactive<ParkCarePrice>({
  id: undefined,
  parkCode: '',
  careTypeCode: '',
  priceType: undefined,
...
const priceRules: FormRules<ParkCarePrice> = {
...
function openCreatePrice(careTypeCode: string) {
  ...
  priceForm.careTypeCode = careTypeCode
  priceDialogVisible.value = true
}

function openEditPrice(row: ParkCarePrice, careTypeCode: string) {
...
async function handlePriceSubmit() {
  ...
  try {
    ...
    priceForm.careTypeCode = priceContext.careTypeCode
    if (priceDialogMode.value === 'create') {
      await createCarePrice(priceForm)
      ElMessage.success('新增成功')
    } else if (priceForm.id) {
      await updateCarePrice(priceForm.id, priceForm)
      ElMessage.success('修改成功')
    }
...
async function handleDeletePrice(row: ParkCarePrice, careTypeCode: string) {
  if (!row.id) return
...
<template>
  <div class="care-tab">
    ...
    <el-form :inline="true" :model="query" @submit.prevent>
      <el-form-item label="照护名称">
        <el-input v-model="query.careTypeName" placeholder="照护名称" clearable @keyup.enter="handleSearch" />
      </el-form-item>
      ...
      <el-form-item>
        ...
        <el-button :icon="'Plus'" @click="openCreateType">新增照护</el-button>
      </el-form-item>
    </el-form>
    ...
    >
      <el-table-column type="expand">
        <template #default="{ row }">
          <div class="price-block" v-loading="priceLoadingMap.get(row.careTypeCode)">
            <div class="price-toolbar">
              <span class="price-title">价格记录（{{ row.careTypeName }}）</span>
              <el-button type="primary" size="small" :icon="'Plus'" @click="openCreatePrice(row.careTypeCode)">
                新增价格
              </el-button>
            </div>
            <el-table :data="priceMap.get(row.careTypeCode) || []" border size="small">
