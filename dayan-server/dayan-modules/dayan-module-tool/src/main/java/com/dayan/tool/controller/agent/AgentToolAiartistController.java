...
import com.dayan.tool.service.ToolAiCreatorStreamService;
...
public class AgentToolAiCreatorController {
    ...
    public R<ToolAiCreatorVO> digest(@PathVariable Long id) {
        return R.ok(pipelineService.digest(id));
    }

    @Operation(summary = "生成策略+5标题")
    @PostMapping("/{id}/strategy")
    public R<ToolAiCreatorVO> strategy(@PathVariable Long id) {
        return R.ok(pipelineService.strategy(id));
    }

    @Operation(summary = "带反馈重出标题（策略锁定）")
    @PostMapping("/{id}/titles/regenerate")
    public R<ToolAiCreatorVO> regenTitles(@PathVariable Long id, @RequestBody(required = false) AiTitleRegenDTO dto) {
    ...
    public R<ToolAiCreatorVO> confirmStrategy(@PathVariable Long id, @RequestBody @Valid AiStrategyConfirmDTO dto) {
        return R.ok(pipelineService.confirmStrategy(id, dto));
    }

    @Operation(summary = "生成大纲")
    @PostMapping("/{id}/outline")
    public R<ToolAiCreatorVO> outline(@PathVariable Long id) {
        return R.ok(pipelineService.outline(id));
    }

    @Operation(summary = "带反馈重生成大纲")
    @PostMapping("/{id}/outline/regenerate")
    public R<ToolAiCreatorVO> regenOutline(@PathVariable Long id, @RequestBody(required = false) AiOutlineRegenDTO dto) {
    ...
    public R<ToolAiCreatorVO> confirmOutline(@PathVariable Long id, @RequestBody @Valid AiOutlineConfirmDTO dto) {
        return R.ok(pipelineService.confirmOutline(id, dto));
    }

    @Operation(summary = "生成正文（非流式，小程序降级用）")
    @PostMapping("/{id}/body")
    public R<ToolAiCreatorVO> body(@PathVariable Long id) {
    ...
    public R<ToolAiCreatorVO> revise(@PathVariable Long id, @RequestBody @Valid AiReviseDTO dto) {
