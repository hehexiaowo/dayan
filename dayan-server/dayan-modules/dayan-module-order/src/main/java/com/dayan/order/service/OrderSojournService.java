 ...
 */
public interface OrderSojournService {

    PageResult<OrderSojournVO> page(OrderSojournQueryDTO query);

    List<OrderSojournVO> list(OrderSojournQueryDTO query);

    OrderSojournVO getDetail(String orderCode);

    /** 查询实体（不存在抛业务异常） */
    OrderSojourn requireOrder(String orderCode);

    /** 创建订单：生成订单号 + 校验金额(room+care+food+other) + 置 order_status=0 → 返回 orderCode */
    String create(CreateOrderSojournDTO dto);
