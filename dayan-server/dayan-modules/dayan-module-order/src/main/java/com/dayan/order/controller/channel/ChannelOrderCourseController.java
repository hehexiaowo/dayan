...
/**
 * Channel 渠道端课程订单接口（订单管理）。
 *
 * <p>路径：{@code /channel-api/order-courses/*}（由 dayan-channel 启动模块的 context-path 拼接）。
 *
 ...
 */
@Tag(name = "Channel 课程订单")
@RestController
@RequestMapping("/order-courses")
@RequiredArgsConstructor
public class ChannelOrderCourseController {

    private final OrderCourseService orderCourseService;

    @Operation(summary = "本渠道课程订单分页列表")
    ...
    public R<OrderCourseVO> getDetail(@PathVariable String orderCode) {
        OrderCourseVO vo = orderCourseService.getDetail(orderCode);
