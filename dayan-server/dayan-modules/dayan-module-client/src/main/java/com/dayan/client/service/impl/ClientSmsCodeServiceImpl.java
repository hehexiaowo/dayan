 ...
 * Client 客户端短信验证码服务实现。
 *
 * <p>验证码 6 位数字，存入 Redis（key=dayan:sms:code:client:{mobile}，TTL 5min）。
 * <p>60 秒重发冷却（key=dayan:sms:cooldown:client:{mobile}，TTL 60s）。
 */
@Slf4j
@Service
@RequiredArgsConstructor
public class ClientSmsCodeServiceImpl implements ClientSmsCodeService {
    ...
    public SmsSendVO sendCode(String mobile, String channelCode) {
        ...
        Long count = accountMapper.selectCount(new LambdaQueryWrapper<ClientAccount>()
                ...
                .eq(ClientAccount::getPhone, mobile));
        if (count == null || count == 0) {
            throw new BusinessException(ErrorCode.NOT_FOUND, "该手机号未关联此渠道的客户账号");
        }
        ...
        log.info("Client 验证码已发送: mobile={}", mobile);
