 ...
 * 采用 内存 Map + storage 双层缓存：同会话内同一区域只拉一次。
 */
...
export async function getRegionGeoJSON(code: string): Promise<GeoJSON.FeatureCollection> {
  ...
  // uni storage 持久缓存（刷新页面后仍命中，跨平台兼容）
  const ssKey = SS_PREFIX + code;
  try {
    const cached = uni.getStorageSync(ssKey);
    if (cached) {
      const json = JSON.parse(cached as string) as GeoJSON.FeatureCollection;
  ...
  memCache.set(code, json);
  try {
    uni.setStorageSync(ssKey, JSON.stringify(json));
  } catch {
    // 存储满或不可用 → 忽略，内存缓存仍生效
  }
  return json;
