...
onMounted(() => {
  smsMobile.value = uni.getStorageSync('client_remember_mobile') || '';
});
...
async function onSmsLogin() {
  ...
  try {
    ...
    uni.setStorageSync('client_remember_mobile', smsMobile.value);
...
async function onPwdLogin() {
  ...
  try {
    ...
    uni.setStorageSync('client_remember_mobile', pwdInput.value);
    uni.showToast({ title: '登录成功', icon: 'success' });
    setTimeout(() => uni.switchTab({ url: '/pages/home/index' }), 500);
  } catch {
    // 错误已提示
