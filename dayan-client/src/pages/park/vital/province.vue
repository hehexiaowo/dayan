...
function handleNavigate(cityOrDistrictCode: string, name?: string) {
  if (isMuni.value) {
    uni.navigateTo({
      url: `/pages/park/vital/district?category=vital&provinceCode=${provinceCode.value}&cityCode=${provinceCode.value.substring(0, 2) + '0100'}&districtCode=${cityOrDistrictCode}`,
    });
  } else {
    const nameQuery = name ? `&cityName=${encodeURIComponent(name)}` : '';
    uni.navigateTo({
      url: `/pages/park/vital/city?category=vital&provinceCode=${provinceCode.value}&cityCode=${cityOrDistrictCode}${nameQuery}`,
    });
  }
}
