...
function handleNavigate(cityOrDistrictCode: string, name?: string) {
  if (isMuni.value) {
    uni.navigateTo({
      url: `/pages/park/care/district?category=care&provinceCode=${provinceCode.value}&cityCode=${provinceCode.value.substring(0, 2) + '0100'}&districtCode=${cityOrDistrictCode}`,
    });
  } else {
    const nameQuery = name ? `&cityName=${encodeURIComponent(name)}` : '';
    uni.navigateTo({
      url: `/pages/park/care/city?category=care&provinceCode=${provinceCode.value}&cityCode=${cityOrDistrictCode}${nameQuery}`,
    });
  }
}
