  ...
  <view class="page">
    ...
    <view class="card">
      <text class="intro-text">
        大雁养老客户端，为家属提供一站式养老服务体验。在这里，您可以便捷地查询全国养老机构、预订旅居养老、管理专属权益、跟进服务进度，让长辈的晚年生活更安心、更舒心。
      </text>
    </view>
    ...
    <view class="card">
      <view v-for="(f, i) in features" :key="i" class="feature-row" :class="{ last: i === features.length - 1 }">
        <view class="feature-dot" :style="{ background: f.color }">
    ...
    <view class="card">
      <view class="link-row" @click="onAgreement('用户协议')">
        <text class="link-text">用户协议</text>
        <text class="link-arrow">›</text>
      </view>
      <view class="link-row last" @click="onAgreement('隐私政策')">
...
const features = [
  { icon: '构', color: 'linear-gradient(135deg,#409eff,#5f8afe)', title: '养老网络', desc: '活力长居 / 照护长居 / 旅居养老，地图下钻精准找机构' },
  { icon: '单', color: 'linear-gradient(135deg,#36cfc9,#5cdbd3)', title: '旅居预订', desc: '在线选房型、定日期，便捷预订心仪机构' },
...
function onAgreement(name: string) {
  uni.showToast({ title: `${name}内容即将上线`, icon: 'none' });
}
</script>
