...
│   ├── dayan-modules/         19 个业务域模块（organ/channel/park/equity/goods/order/agent/lead/content/tool/course…）
...
| **渠道端** | 6 菜单组约 40 页全量（机构库/内容/商品/权益/客户/线索，另系统管理·知识仓库），渠道级能力闸（can_manage）与白名单配置 |
...
| **知识仓库** | 百炼 RAG 平台库+渠道库、文档上传解析导入、检索测试与问答、channel 端同步管理（类目/标签/上传设置/索引配置）、AI 创作素材勾选精准召回 |
...
- **知识库**：admin 系统管理→知识仓库（平台库 + 每渠道一库），上传文档后需建库（百炼索引）；channel 端系统管理→知识仓库同步管理
